...
//Agregar mmétodos para PUT y DELETE 
        switch r.Method {
        case http.MethodGet:
            h.getBook(w, r, id)
        case http.MethodPut:
...
// Método para actualizar un libro (PUT)
...
func (h *BookHandler) deleteBook(w http.ResponseWriter, r *http.Request, id int) {
	...
	w.WriteHeader(http.StatusNoContent) // respuesta 204 No Content
}
